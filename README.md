...
Собрать свою версию из исходников:
...
```
--onedir - для одного каталога + exe (Немного быстрее запускается)
--onefile - для одного exe (Запускается дольше)
```

Результат: pdfpages_pro.exe — запускается без Python!
...
## 🔧 Настройка
 ...
 Настройки в файле *config.yaml*, можно редактировать в тексовом редакторе:

- Допуск распознавания

```text
tolerance_mm: 5.0  # прогрешность ±5мм 
```
